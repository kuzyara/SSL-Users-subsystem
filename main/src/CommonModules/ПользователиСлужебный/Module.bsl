...
Функция СсылкаОбъекта(Знач Объект, ЭтоНовый = Неопределено) Экспорт
	...
	Если ЭтоНовый Тогда
		...
		Если НЕ ЗначениеЗаполнено(Ссылка) Тогда
			...
			Менеджер = Справочники.Пользователи;//ОбщегоНазначения.МенеджерОбъектаПоСсылке(Объект.Ссылка);
...
Функция ВозможноИзменитьПароль(Пользователь, ДополнительныеПараметры = Неопределено) Экспорт
	...
	//РеквизитыПользователя = ОбщегоНазначения.ПолучитьЗначенияРеквизитовСправочника(
		//Пользователь, "Ссылка, Недействителен, ИдентификаторПользователяИБ, Подготовлен");
	РеквизитыПользователя = Новый Структура;
	РеквизитыПользователя.Вставить("Ссылка", Пользователь);
	РеквизитыПользователя.Вставить("ИдентификаторПользователяИБ", Пользователь.ИдентификаторПользователяИБ);
	
	Если РеквизитыПользователя.Ссылка <> Пользователь Тогда
		РеквизитыПользователя.Ссылка = Справочники.Пользователи.ПустаяСсылка();//ОбщегоНазначения.МенеджерОбъектаПоСсылке(Пользователь).ПустаяСсылка();
	...
	//	+ "дней";//ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоДней,
	//		//"", НСтр("ru = 'день,дня,дней,,,,,,0'"));
	//
	//ДополнительныеПараметры.Вставить("ТекстОшибки", СтрШаблон(
	//	НСтр("ru = 'Пароль можно будет сменить только через %1.'"), ЧислоИПредмет));
	//ДополнительныеПараметры.Вставить("ТекстОшибки", "Неизвестная ошибка");
...
Функция ОбработатьНовыйПароль(Параметры) Экспорт
	...
	Если ТипЗнч(Параметры.Пользователь) = Тип("СправочникОбъект.Пользователи")
	 ...
	 Тогда
		...
		Если ТипЗнч(Параметры.Пользователь) = Тип("СправочникОбъект.Пользователи")
		   //И Параметры.Пользователь.Служебный 
		   Тогда
	...
	Если ПользователиСлужебный.Настройки().ОбщиеНастройкиВхода Тогда
		//Если ТипЗнч(Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		//	НастройкиВхода = ПользователиСлужебный.Настройки().ВнешниеПользователи;
		//Иначе
			НастройкиВхода = ПользователиСлужебный.Настройки().Пользователи;
		//КонецЕсли;
	...
	Попытка
		...
		Если ПользователиСлужебный.Настройки().ОбщиеНастройкиВхода
...
Функция Настройки() Экспорт
	...
	Возврат ВсеНастройки; //ОбщегоНазначения.ФиксированныеДанные(ВсеНастройки);
...
Процедура НачатьОбработкуПользователяИБ(ПользовательОбъект,
	...
	УровеньДоступа = ПользователиСлужебный.УровеньДоступаКСвойствамПользователя(ПользовательОбъект, ПараметрыОбработки);
	...
	//Если //ПараметрыОбработки.СтарыйПользователь.Подготовлен <> ПользовательОбъект.Подготовлен И 
	//	Не УровеньДоступа.ИзменениеРазрешенияНаВход Тогда
	//	
...
Функция ЗначенияПараметровСеансаТекущийПользователь()
	...
	Если СведенияОПользователе.ТекущийПользователь        = Неопределено
	 //Или СведенияОПользователе.ТекущийВнешнийПользователь = Неопределено 
	 Тогда
...
Процедура СкопироватьНастройкиПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	ИдентификаторКопируемогоПользователяИБ = ПараметрыОбработки.ЗначениеКопирования.ИдентификаторПользователяИБ;
...
Функция ТребуетсяСменитьПароль(ОписаниеОшибки = "", ПриЗапуске = Ложь, РегистрироватьВЖурнале = Истина) Экспорт
...
Процедура ЗапомнитьСвойстваПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	СтарыйПользователь = Новый Структура(Поля);
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	Пользователи.Ссылка = &Ссылка";
	Запрос.Параметры.Вставить("Ссылка", ПользовательОбъект.Ссылка);
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(СтарыйПользователь, ВыборкаДетальныеЗаписи);
	КонецЕсли;;
...
Процедура ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки) Экспорт
	...
	Пользователи.УстановитьСвойстваПользователяИБ(ИдентификаторПользователяИБ, ОписаниеПользователяИБ, 
		СоздатьНовогоПользователяИБ, Ложь);//ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.ВнешниеПользователи"));
...
Функция УдалитьПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Возврат ОписаниеПользователяИБ.РезультатДействия;
...
Процедура ПроверитьИзмененияРеквизитовПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	Если Не ДанныеСовпадают(АвтоРеквизиты.СвойстваПользователяИБ,
